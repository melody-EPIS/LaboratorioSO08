...
int produce(pthread_t self){
    ...
    printf("Productor %d productor %d \n",i+1,p);
    return p;
}
...
void consume(int p,pthread_t self){
    ...
    for(i=0;i<=buf_pos;++i)
        printf("%d ",*(buf+i));
    printf("\nConsumidor %d consumidor %d \vVoy a pagar por: %d\n",i+1,p,buf_pos);
...
int main(void){
    ...
    printf("Ingresar numero de productores:");
    scanf("%d",&prod_count);
    producers = (pthread_t*) malloc(prod_count*sizeof(pthread_t));

    printf("Ingresar numero de consumidores:");
    scanf("%d",&con_count);
    consumers = (pthread_t*) malloc(con_count*sizeof(pthread_t));

    printf("Ingresar capacidad de buffer");
    scanf("%d",&buf_len);
    ...
    for(i=0;i<prod_count;i++){
        ...
        if(err != 0){
            printf("Error al asignar productor %d: %s\n",i+1,strerror(err));
        }else{
            printf("Productor creado satisfactoriamente %d\n",i+1);
        }
    }
    ...
    for(i=0;i<con_count;i++){
        ...
        if(err != 0){
            printf("Error al crear consumidor%d: %s\n",i+1,strerror(err));
        }else{
            printf("Consumidor creado satisfactoriamente %d\n",i+1);
        }
    }
